/*// Create Pixel Shader
	...
	D3DReadFileToBlob(string_to_wstring(pixel_shader_file_name).c_str(), &blob);
	...
	D3DReadFileToBlob(string_to_wstring(vertex_shader_file_name).c_str(), &blob);
	...
	context->DrawIndexed((UINT)vertices.size(), 0u, 0u);*/
/*
...
void GraphicsScene::draw() {
	for (Object object : *objects) {
		...
		Microsoft::WRL::ComPtr<ID3DBlob> blob;
		Microsoft::WRL::ComPtr<ID3D11PixelShader> pixel_shader;
		D3DReadFileToBlob(string_to_wstring(object.appearance.pixel_shader).c_str(), &blob);
		HANDLE_POSSIBLE_EXCEPTION(device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &pixel_shader));
		...
		// Create Vertex Shader
		Microsoft::WRL::ComPtr<ID3D11VertexShader> vertex_shader;
		D3DReadFileToBlob(string_to_wstring(object.appearance.vertex_shader).c_str(), &blob);
		HANDLE_POSSIBLE_EXCEPTION(device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &vertex_shader));
...
void GraphicsScene::set_vertex_buffer(Object object) {
	...
	buffer_description.ByteWidth = sizeof(Vertex) * object.appearance.mesh.vertices.size() + sizeof(Vertex);
...
void GraphicsScene::set_index_buffer(Object object) {
	...
	index_buffer_description.ByteWidth = sizeof(UINT) * object.appearance.mesh.indices.size() + sizeof(UINT);
